...
String agree=(String)session.getAttribute("agree");
       ...
       try
        ...
        String sql="insert into server values('"+user+"','"+firstname+"','"+lastname+"','"+date+"','"+sport+"','"+age+"','"+telephone+"','"+password2+"','"+email2+"','"+comments+"','"+agree+"')";
		
        int x=st.executeUpdate(sql);
		if(x!=0)
		{
		response.sendRedirect("userregister.jsp?message=success,you are successfully registered");
		}
		else
		{
			response.sendRedirect("userregister.jsp?message=fail");
		}
        con.close();
        st.close();
        }
